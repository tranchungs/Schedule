...
class Schedule {
  ...
  ConvertData(input) {
    for (const item of input) {
      ...
      const regexDetails = /(Từ(?<Start>.*?)đến(?<End>.*?):) <b>\((?<NumberAddress>\d{0,4})\)<\/b><br>(?<Detail>&nbsp;&nbsp;&nbsp;<b>Thứ \d tiết [\d|,]+ [\(LT\)|\(TH\)]+<\/b><br>){0,4}/gms;
      ...
      for (const itemDetail of Detailshtml) {
        ...
        let timeStart = new Date(timeStartSplit[2],timeStartSplit[1]-1,timeStartSplit[0]).getTime();
        let timeEnd = new Date(timeEndSplit[2],timeEndSplit[1]-1,timeEndSplit[0]).getTime();
        ...
        for (const itemdataLearn of dataLearn) { // duyệt qua tuần học
